...
/*!
 * \file
 * \brief Test to find values in sets.
 */
#include <cassert>
#include <cstddef>
#include <exception>
...
class find_values_fixture : public stat_bench::FixtureBase {
public:
    find_values_fixture() {
...
STAT_BENCH_GROUP("find_values").add_parameter_to_time_line_plot_log("size");
...
STAT_BENCH_CASE_F(find_values_fixture, "find_values", "unordered_set") {
    ...
    STAT_BENCH_MEASURE() {
        for (std::size_t i = 0; i < size_; ++i) {
            const auto& key = keys_.at(i);
            stat_bench::do_not_optimize(set.count(key));
        };
    };
}

// NOLINTNEXTLINE
STAT_BENCH_CASE_F(find_values_fixture, "find_values", "open_address_st") {
    ...
    STAT_BENCH_MEASURE() {
        for (std::size_t i = 0; i < size_; ++i) {
            const auto& key = keys_.at(i);
            stat_bench::do_not_optimize(set.has(key));
        };
    };
}
