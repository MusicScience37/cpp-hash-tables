...
/*!
 * \file
 * \brief Definition of multi_open_address_map_st class.
 */
#pragma once
...
/*!
 * \brief Class to save key-value relations in a hash table made of multiple
...
class multi_open_address_map_st {
public:
    //! Type of keys.
    ...
    using table_type = tables::multi_open_address_table_st<value_type, key_type,
        extract_key_type, hash_type, key_equal_type, allocator_type>;

    /*!
     * \brief Constructor.
     */
    multi_open_address_map_st() : table_() {}
     ...
     */
    explicit multi_open_address_map_st(size_type min_num_tables,
     ...
     */
    multi_open_address_map_st(const multi_open_address_map_st&) = default;

    /*!
     * \brief Move constructor.
     */
    multi_open_address_map_st(multi_open_address_map_st&&)
     ...
     * \return This.
     */
    auto operator=(multi_open_address_map_st&&)
#ifndef HASH_TABLES_DOCUMENTATION
     ...
     * \brief Destructor.
     */
    ~multi_open_address_map_st() noexcept = default;
    ...
    auto get_or_create(const key_type& key, Args&&... args) -> mapped_type& {
        return table_
            .get_or_create(key, std::piecewise_construct,
                std::forward_as_tuple(key),
                std::forward_as_tuple(std::forward<Args>(args)...))
            .second;
    }
     ...
     */
    auto try_get(const key_type& key) -> mapped_type* {
        auto* value = table_.try_get(key);
        if (value == nullptr) {
            return nullptr;
        }
